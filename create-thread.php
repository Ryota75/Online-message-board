<?php
    ...
    if(isset($_POST['thread_title']) && $_POST['thread_title'] != ""){
        $title = $_POST['thread_title'];
        $sql = "INSERT INTO threadTitle (title) VALUES (:title)";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':title', $title, PDO::PARAM_STR);
        $stmt->execute();
        
        $sql = "SELECT MAX(id) FROM threadTitle";
        $stmt = $pdo->query($sql);
        $result = $stmt->fetch();
        
        $title_id = $result["MAX(id)"];
        
        $filename = $title_id.".php";
        $fp = fopen($filename,"w");
        fwrite($fp,
'<!DOCTYPE html>
<html lang="ja">
...
<body>
    <?php 
        ...
        if(isset($_POST["submit_post"])){
            ...
            if($_POST["comment"] != ""){
                ...
                //投稿番号
                $sql = "SELECT number FROM tbtest WHERE threadID='.$title_id.' ORDER BY number DESC LIMIT 1";
                $stmt = $pdo->query($sql);
                $result = $stmt->fetch();
    ...
    <div class="display">
        ...
        fclose($fp);
        header("Location:https:/tb-250260/list-app/".$filename);
        exit();
        }
    ?>